  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.from('.hero__eyebrow', { opacity: 0, y: 20, duration: 1, delay: 0.3 })
        .from('.hero__tagline-word', { opacity: 0, y: 50, duration: 1.2, stagger: 0.08 }, '-=0.6')
        .from('.hero__sub', { opacity: 0, y: 20, duration: 1 }, '-=0.7')
        .from('.hero__cta', { opacity: 0, y: 20, duration: 1 }, '-=0.7')
        .from('.hero__indicators, .hero__scroll-chevron', { opacity: 0, duration: 1.2 }, '-=0.8');
    }, containerRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section className="hero" ref={containerRef}>
      ...
      <div className="hero__content">
        ...
        <h1 className="hero__tagline">
          <span className="hero__tagline-line">
            {line1.map((word, i) => (
              <span key={i} className="hero__tagline-word">{word}{i < line1.length - 1 ? '\u00A0' : ''}</span>
            ))}
          </span>
          <span className="hero__tagline-line">
            {line2.map((word, i) => (
              <span key={i} className="hero__tagline-word"><em>{word}</em>{i < line2.length - 1 ? '\u00A0' : ''}</span>
            ))}
          </span>
        </h1>
      ...
      <div className="hero__indicators">
        {heroImages.map((_, i) => (
          <button key={i} className={`hero__dot ${index === i ? 'hero__dot--active' : ''}`}
